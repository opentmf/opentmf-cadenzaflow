...
| Provided API Specification(s) | [`docs/openapi.yaml`](openapi.yaml) — the complete surface in one generated document: the engine REST API rebased under `/engine-rest` plus the service's own additions (`/engine-rest/extensions/**`, management endpoints), merged by `ci/merge-openapi.py` from the engine's spec artifact and the source fragment `ci/openapi-service.yaml`. See [§11.1](#111-api-contract-specification) |
...
| Database Dependency | PostgreSQL. The engine owns the schema — 49 `ACT_*` tables — and creates the missing ones at startup; it does not alter existing ones. The service defines no tables of its own; its incident-operations extension reads (never writes) the engine's runtime tables with two hand-written, index-aligned selects ([README §6.6](../README.md#66-uc-40--uc-41--grouped-incidents-are-reported-and-retried)). [README §7](../README.md#7-data), full column catalogue in [`docs/database-schema.md`](database-schema.md) |
...
| Other Dependencies | `openid-rbac-security` (token validation, RBAC), `cadenzaflow-keycloak-4` / `cadenzaflow-entra-identity-4` (identity providers), GraalJS (script tasks), Spin (large variables), `tmf630-toolkit-paging-sorting-core` + `spring-data-commons` (TMF-630 paging for the incident list) |
...
| § | Scope | Where |
|---|---|---|
| 6 | Web-service use cases | [README §6.1](../README.md#61-uc-05--uc-07--a-rest-call-is-authorized) — authorization of a REST call, with its sequence diagram; [README §6.6](../README.md#66-uc-40--uc-41--grouped-incidents-are-reported-and-retried) — the incident-operations extension (call-tree grouping, propagation dedup, group retry) |
...
[`docs/openapi.yaml`](openapi.yaml) (OpenAPI 3.2.0), generated by `ci/merge-openapi.py`
(service surface authored in `ci/openapi-service.yaml`, engine surface from the
engine's own spec artifact at the exact embedded version). One document covers the
whole surface: every engine REST endpoint rebased under `/engine-rest`, the
service's own additions under `/engine-rest/extensions/**` (grouped under the
`Extensions: Incidents` tag), and the management-port endpoints — with the deployed
bearer-JWT security model. The engine half remains the upstream platform's
contract; this component authors only the fragment.
...
[README §6.3](../README.md#63-uc-10--uc-13--a-worker-runs-an-external-task); the
incident-operations extension (grouped report, windowed retry, TMF-630 list, with
full parameter tables) is [README §9.3](../README.md#93-incidents).
...
org.opentmf.cadenzaflow
  ...
  extensions/    # the incident-operations extension (/engine-rest/extensions/**),
                 # layered: resource -> service -> repository, models by feature
    resource/    # the three JAX-RS resources + TMF-630 paging glue
    service/     # roll-up, engine-batch retry, list query building/enrichment
    repository/  # hand-written SQL, its row type, engine-cache lookups
    model/incident/group/ , model/incident/list/   # the serialized API vocabulary
    config/      # the two @ConfigurationProperties classes
    util/        # EngineRestDateUtil (dual-format timestamp parsing)
```

The service owns no entities and no tables; its only REST resources of its own are
the three incident-operations extensions, mounted as JAX-RS resources inside the
engine's Jersey application (registered by `JerseyConfig`) rather than as a Spring
MVC surface — Jersey owns `/engine-rest/*`. Architectural boundaries enforced by
ArchUnit: provider-specific identity libraries stay inside the two adapters in
`config` (so the rest of the code cannot quietly become Keycloak-only or
Entra-only), and the general coding rules (no field injection, explicit names on
request-binding annotations, SLF4J-only) apply to the extension package like
everywhere else.
...
| `spring.threads.virtual.enabled: true` | **Not adopted** | Re-examined in depth (2026-08-19) and the decision stands, but **the two reasons originally given here were wrong** and are recorded as such: the Camunda job executor builds its own `ThreadPoolTaskExecutor`, so the property never reaches it, and the engine's ThreadLocal context stacks are popped in a `finally`, so they unwind correctly per request. The real reasons are: (1) **it would buy nothing** — the connection pool binds roughly 7× before Tomcat threads do, and `fetchAndLock`, the one path where thread-per-request would matter, is already async; (2) **`HikariCP` 7.0.2 pre-dates the virtual-thread fix** — its `ConcurrentBag` spin loops call `Thread.yield()`, which re-submits a virtual thread instead of releasing its carrier; the `isVirtual` branch first ships in 7.1.0; (3) **class-initialization pinning is not fixed on JDK 25** — JEP 491 addressed monitor pinning only, and `<clinit>` waits still pin (fixed in JDK 26), which matters for a cold start that lazily loads as many classes as this one; (4) **GraalJS marks virtual-thread support experimental** and disables the `sandbox.MaxCPUTime` / `MaxHeapMemory` limits under it. Revisit when HikariCP ≥ 7.1.0 and JDK ≥ 26 are in place, and even then only if measurement shows the pool is not the constraint. It remains a runtime property, so a deployment can still enable and measure it without a rebuild |
...
- [2] [docs/openapi.yaml](openapi.yaml) — the API contract (generated; source fragment ci/openapi-service.yaml)
